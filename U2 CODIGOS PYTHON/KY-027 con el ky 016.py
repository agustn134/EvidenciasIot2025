...
MQTT_TOPIC_PUBLISH = "utng/sensor"      # Tema para publicar estado del KY-027
MQTT_PORT = 1883
...
# Configuración del KY-027 (Sensor de inclinación)
TILT_PIN = 34  # GPIO para el sensor KY-027
tilt_sensor = Pin(TILT_PIN, Pin.IN)  # Configurar como entrada digital
...
def read_tilt_sensor():
    return tilt_sensor.value()  # Devuelve 0 o 1
...
# Estado anterior del sensor de inclinación
previous_tilt_state = None
...
while True:
    ...
    current_tilt_state = read_tilt_sensor()
    
    # Si el estado ha cambiado, publicar el nuevo estado en MQTT
    if current_tilt_state != previous_tilt_state:
        previous_tilt_state = current_tilt_state
        print(f"Estado del sensor de inclinación: {current_tilt_state}")
        client.publish(MQTT_TOPIC_PUBLISH, str(current_tilt_state))
        
        # Cambiar el color del LED RGB según el estado del sensor
        if current_tilt_state == 1:
            set_rgb_color(255, 0, 0)  # Rojo si está inclinado
        else:
            set_rgb_color(0, 255, 0)  # Verde si no está inclinado
